  ...
  <!-- Cricket Match -->
    ...
    if(isset($_POST['match_order'])){
      if ($_SERVER['REQUEST_METHOD']=='POST') {
        ...
        $check="SELECT * FROM `cricket_order` WHERE `username`='$username' AND `email`='$email' AND `match_name`='$match_name' AND `team_one`='$team_one' AND `team_two` = '$team_two'";
        $result = mysqli_query($conn, $check);
        $num = mysqli_num_rows($result);
        if($num>0){
          $sql = "UPDATE `cricket_order` SET
              `username`='$username',
              `email`='$email',
          ...
          WHERE `username`='$username' AND `email`='$email' AND `match_name`='$match_name' AND `team_one`='$team_one' AND `team_two` = '$team_two';";
                $book=mysqli_query($conn, $sql);
          $showAlreadyBuyAlert=true;
        }
        else{
          $sql = "INSERT INTO `cricket_order`
                (`username`, `email`, `match_name`, `team_one`,`team_two`, `price`, `tickets`, `total`, `card_number`, `card_pin`, `booking_time`)
                VALUES
                ('$username', '$email', '$match_name', '$team_one','$team_two', '$price', '$tickets', '$total', '$card_no_hash', '$card_pin_hash', current_timestamp())";
                $book=mysqli_query($conn, $sql);
            if($book){
              $showAlert=true;
            }else{
...
<?php
  ...
  $num=mysqli_num_rows($result);
                     if ($num==0) {
                      echo ' 
                      <section id="cricket" class="d-flex align-items-center justify-content-center">
    
                        <div class="container" data-aos="fade-up">

                          <div class="row justify-content-center" data-aos="fade-up" data-aos-delay="150">
                            <div class="col-xl-6 col-lg-8">
                              <h1>Cricket Matches<span>!</span></h1>
                              
                            </div>
                          </div>

                          <h2><span>"</span> There are no Upcoming Matches Available <span>"</span></h2>
                      </section>
                      ';
                     }
                     else{
                      echo '
                      <section id="cricket2" >
                      <h2 class="heading"><strong>Cricket Matches<span class="heading_span">!</span></strong> </h2>
                      ';
                     }
  ?>
    ...
    while($row=mysqli_fetch_assoc($result)){
      ?>
        ...
        <div class="card">
            <img src="admin/cricket/<?php echo $row['poster']?>" alt="" class="poster">
            <div class="intro" >
              <img src="admin/cricket/<?php echo $row['logo']?>" alt="" class="inner_poster">
              <strong><p><span>Match:</span><?php echo $row['match_name']?></p></strong>
                <p style="font-size: 10px!important;"><span>Teams:</span><?php echo $row['team_one']?><b><span style="padding-right:0;"> vs </span></b><?php echo $row['team_two']?></p>
                <p><span>Location:</span><?php echo $row['location']?></p>
                <p><span>Date:</span><?php echo $row['date']?></p>
                <p><span>Time:</span><?php echo $row['time']?></p>
                <p style="font-size: 12px!important;"><span>Address:</span><?php echo $row['address']?></p>
                <strong><p><span>Price:</span>Rs. <?php echo $row['price']?> - person</p></strong>
              <a href="cricket_ticket.php?eventid=<?php echo $row['id']?>"><button class="buy-button"><strong>Buy Now!</strong></button></a>
            </div>
        </div>
    <?php
    }
