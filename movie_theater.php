...
<body>
  ...
  <!-- Movie Theater -->
  <section id="theater" class="d-flex align-items-center justify-content-center">
    <div class="container" data-aos="fade-up">
    
      <div class="row justify-content-center" data-aos="fade-up" data-aos-delay="150">
        <h1>Movie Theaters<span>!</span></h1>
        <div class="col-xl-6 col-lg-8">
          <?php
          $showAlreadyBuyAlert=false;
          $showAlert=false;
          $error=false;
          if (isset($_POST['movie_order'])) {
            if ($_SERVER['REQUEST_METHOD'] == 'POST'){
              ...
              $check="SELECT * FROM `movie_order` WHERE `name`='$name' AND `email`='$email' AND `theater_name`='$theater_name' AND `movie_name`='$movie_name' AND `selected_show_time`='$selected_show_time'";
              $result=mysqli_query($conn,$check);
              $num=mysqli_num_rows($result);
              if($num>0){
                $sql = "UPDATE `movie_order` SET
                `name`='$name',
                `email`='$email',
                ...
                WHERE `name`='$name' AND `email`='$email' AND `theater_name`='$theater_name' AND `movie_name`='$movie_name' AND `selected_show_time`='$selected_show_time'";
                $book=mysqli_query($conn, $sql);
                $showAlreadyBuyAlert=true;
              }
              else{
              $sql = "INSERT INTO `movie_order`(`name`, `email`, `theater_name`, `movie_name`, `selected_show_time`, `price`, `tickets`, `total`, `card_number`, `card_pin`, `booking_time`) 
                                        VALUES
                                        ('$name','$email','$theater_name','$movie_name','$selected_show_time','$price','$tickets','$total','$card_no_hash', '$card_pin_hash', current_timestamp())";
                                        $book=mysqli_query($conn, $sql);
                                        if($book){
                                          $showAlert=true;
                                        }else{
    ...
    if($showAlert){
      ...
      <?php
        ...
        while($row=mysqli_fetch_assoc($result)){
              ?>
        <div class="col-xl-2 col-md-4"id="t-box">
          <a href="<?php echo $row['file_name'] ?>.php?theatername=<?php echo $row['theater_name'] ?>&?theaterid=<?php echo $row['id'] ?>" class="link">
          <div class="icon-box">
            <img src="admin/movie_theaters/<?php  echo $row['logo'];?>" alt="Atrium" class="img-fluid">
            <h3><?php  echo $row['theater_name'];?></h3>
          </div></a>
        </div>
        <?php
        }
