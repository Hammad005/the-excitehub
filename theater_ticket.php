...
<body>
  ...
  <!-- theater tickets -->
  <section id="theater_tickets">
    <div class="main-block">
      <div class="left-part">
        <a href="movie_theater.php"><img src="logo/default3.png" alt="" style="padding-right: 20px;"></a>
      </div>
      <?php
      if(isset($_GET['id'])){
        $id = $_GET['id'];
      ...
      ?>
      <form  action="movie_theater.php" method="POST" onsubmit="return validateForm()">
        <div class="title">
        ...
        <div class="info">
          ...
          <input type="text" id="phoneNumber" name="theater_name" placeholder="Movie Theater" value="<?php echo $theater_name ?>" readonly required>
          <label>Movie:</label>
          <input type="text" id="phoneNumber" name="movie_name" placeholder="Movie Theater" value="<?php echo $row['movie_name']?>" readonly required>
          <label class="inline-input">
  Show Times: <br>
  
  <input type="text" id="dateofshow1" name="show_time_1" value="<?php echo $row['date_of_show_1'] ?>" readonly placeholder="date_of_show_1" required>
  
  <label class="inline-input"><b style="padding-right:10px;">:</b> 
    <input type="radio" id="firstshowtimeofshow1" name="selected_show_time" value="<?php echo $row['date_of_show_1'] ?> '' <?php echo $row['first_show_time_of_show_1'] ?>" readonly required>
    <label for="firstshowtimeofshow1"> <?php echo $row['first_show_time_of_show_1'] ?></label>
    
    <input type="radio" id="secoundshowtimeofshow1" name="selected_show_time" value="<?php echo $row['date_of_show_1'] ?> '' <?php echo $row['secound_show_time_of_show_1'] ?>" readonly required>
    <label for="secoundshowtimeofshow1"> <?php echo $row['secound_show_time_of_show_1'] ?></label>
  </label>
  <br>
  <input type="text" id="dateofshow2" name="show_time_2" value="<?php echo $row['date_of_show_2'] ?>" readonly placeholder="date_of_show_2" required>
  
  <label class="inline-input"><b style="padding-right:10px;">:</b>
    <input type="radio" id="firstshowtimeofshow2" name="selected_show_time" value="<?php echo $row['date_of_show_2'] ?> '' <?php echo $row['first_show_time_of_show_2'] ?>" readonly required>
    <label for="firstshowtimeofshow2"><?php echo $row['first_show_time_of_show_2'] ?></label>
    
    <input type="radio" id="secoundshowtimeofshow2" name="selected_show_time" value="<?php echo $row['date_of_show_2'] ?> '' <?php echo $row['secound_show_time_of_show_2'] ?>" readonly required>
    <label for="secoundshowtimeofshow2"><?php echo $row['secound_show_time_of_show_2'] ?></label>
  </label>
...
<?php
          ...
          $dis = $query_fetch['dis_movie'];
          }
          $query="SELECT * FROM `theaters` WHERE `theater_name` = '$theater_name'";
          $t_result = mysqli_query($conn, $query);
          $p_row = mysqli_fetch_assoc($t_result);
          ?>
          <label>Ticket Price :</label>
          <input type="number" id="price" name="price" value="<?php echo $p_row['movie_ticket'] ?>" readonly placeholder="Ticket Price" required>
          <script>
            var price = <?php echo $p_row['movie_ticket']?>;
          ...
          <?php
                    if($sub>0){
                        echo '<label style="text-align: right;"><small>Because you have subscribed to the '. $package .' you received a '. $dis .'% discount on the movie ticket.</small></label>';
                      }
          ?>
          <label>Tickets:</label>
        ...
        <button type="submit" class=" mt-2" name="movie_order"><strong>Book Now!</strong></button>
      </form>
    </div>
</section>
  <!-- end theater tickets -->
  ...
  <script>
    ...
    const showTime1 = document.querySelector('input[name="selected_show_time"]:checked');
    
    if (!showTime1) {
        alert('Please select at least one show time.');
        return false;
    }
    return true;
}
